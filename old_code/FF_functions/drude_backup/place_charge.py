...
# Lib for Grid_gen
sys.path.append('/'.join(os.path.abspath(__file__).split('/')[:-2])+'/Grid_gen/bin/')
...
def main(argv):
    ...
    parser.add_argument('-gens', dest='gens', default=2,
                        help = 'generation for TAFFI atomtype')
    ...
    args.size_factor = float(args.size_factor)
    args.density_scale_factor = float(args.density_scale_factor)
    args.criteria = float(args.criteria)
    args.extra = float(args.extra)
    ...
    bond_mat = find_lewis(elements, adj_mat, q_tot=0, verbose=False)

    # Find modes (returns all modes in canonicalized format)
    #bonds_tmp,angles_tmp,dihedrals_tmp,one_fives_tmp,bond_types_tmp,angle_types_tmp,dihedral_types_tmp,one_five_types_tmp = Find_modes(adj_mat,atom_types,bond_mat,return_all=1)
    bonds_tmp,angles_tmp,bond_types_tmp,angle_types_tmp = Find_modes_baonly(adj_mat,atom_types,bond_mat,return_all=1)

    if(args.test):
         test_pt = grid_gen(elements,geo,6,size_factor,extra)
         return
    ...
    for atom in special:
         ...
         for i in range(0,5):
            ...
            if count[i] >1:
               ...
               #index = [intersec_tmp_a.index(intersec_tmp_a[begin:end])]
               index = []
...
def GetInchi(Elements,Geometry): #convert geo2inchi key
    ...
    fid = open('{}/inchi.xyz'.format(ori_dir),'w')
    fid.write('{}\n\n'.format(len(Elements)))
    ...
    # remove xyz file
    os.remove("{}/inchi.xyz".format(ori_dir))

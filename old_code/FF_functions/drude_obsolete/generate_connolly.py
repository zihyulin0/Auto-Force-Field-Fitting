...
def fun(option,geo_dict):
    ...
    for file in geo_dict:        
        ...
        if(test):
            ...
            return
        scale_factors = [ 30, 50, 60 ]
        density_scales = [ 1.3, 0.6, 0.2]
        grid_points = {}
        total_points = 0
        for count_i,i in enumerate(scale_factors):
            grid_points[i] = grid_gen(geo_dict[file]["elements"],geo_dict[file]["geo"],arg_density,scale_factors[count_i],density_scales[count_i])
        ...
        with open('connolly_vis.xyz','w') as f:
            f.write("{} \n\n".format(total_points+len(geo_dict[file]["elements"])))
            for count_i,i in enumerate(geo_dict[file]["geo"]):
               f.write("{} {} {} {}\n".format(geo_dict[file]["elements"][count_i],i[0],i[1],i[2]))
            tmp = ['F', 'Cl', 'Br']

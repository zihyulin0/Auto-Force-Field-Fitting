...
def fun(config,other_opt,argv):
    ...
    print("PROGRAM CALL: python gen_md_for_CHARMM_new.py {}\n".format(' '.join([ i for i in argv])))
    ...
    Data_single = get_data(FF_all,coord_files,[1],q_list,gens,Improper_flag=improper_flag,force_read_opt=force_read_opt,remove_multi=remove_multi)
    alpha_dict = {}
    for i in Data_single:
         print(Data_single[i]['Charges'])
         for count_j,j in enumerate(Data_single[i]['Atom_types']):
            alpha_dict[j] = Data_single[i]['alphas'][count_j]
    ...
    Pack_Box(Data_single,Box_offset=0,Density=density,N_Density=N_density)
    Elements_2,Atom_types_2,Geometry_2,Bonds_2,Bond_types_2,Angles_2,Angle_types_2,Dihedrals_2,Dihedral_types_2,Impropers_2,Improper_types_2,Charges_2,Molecule_2,Molecule_names_2,Adj_mat_2,Sim_Box_2,b_min_2 = \
    ...
    Atom_type_dict,fixed_modes = Write_CHARMM(Filename,Atom_types,Sim_Box,Elements,Geometry,Bonds,Bond_types,Bond_params,Angles,Angle_types,Angle_params,\
                                            Dihedrals,Dihedral_types,Dihedral_params,Impropers,Improper_types,Improper_params,Charges,VDW_params,Masses,Molecule,alpha_dict,Atom_types_2,Geometry_2,Sim_Box_2,Improper_flag = improper_flag,npert=npert)
...
def Write_CHARMM(Filename,Atom_types,Elements,Geometry,Bonds,Bond_types,Bond_params,Angles,Angle_types,Angle_params,Dihedrals,Dihedral_types,Dihedral_params,\
               Impropers,Improper_types,Improper_params,Charges,VDW_params,Masses,alpha_dict,LP_params,Improper_flag=False,polar_flag=False,LP_flag=False,overwrite=False,npert=2,ascale=1):
    ...
    with open(Filename+'/'+'lin.prm','w') as g:
        ...
        for i in sorted(set(Dihedral_types)):
            for count_j,j in enumerate(Dihedral_params[i]):
                if count_j == 0  and j != 'opls':
                    print("ERROR: currently only support opls dihedral style") 
                if count_j == 1:
        ...
        g.write("cutnb 14.0 ctofnb 12.0 ctonnb 10.0 eps 1.0 e14fac 0.0 wmin 1.5\n")
        ...
        for count_i,i in enumerate(sorted(set(Atom_types))):     
            g.write("{}  0.000000 {:<20.6f}   {:<20.6f}\n".format(Atom_type_dict2[Atom_type_dict[i]],VDW_params[(i,i)][1]*(-1),VDW_params[(i,i)][2]*(2**(1.0/6))/2))
    ...
    with open(Filename+'/'+Filename.split('/')[-1]+'.inp','w') as f:
        ...
        f.write("scalar charge set 0.5 select segid CAL end\n\n")

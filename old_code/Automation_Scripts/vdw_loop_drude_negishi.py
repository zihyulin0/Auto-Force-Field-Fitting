...
def main(argv):
    ...
    parser.add_argument('-polar_dir', dest='polar_dir', default='',
                        help = 'directory that polar fit is run in, that has intra_only.db')
    parser.add_argument('-current_gen',type=int, dest='current_gen', default=0,
                        help = 'current parametrization generation')
    ...
    # c['ff'] =  nonpolar.db -> md sampling needs to use the complete nonpolar.db
    global polar_dir
    polar_dir = args.polar_dir
    if polar_dir == '':
        print("ERROR: need to specify polar dir")
        quit()
    if args.current_gen == 0:
        print("ERROR: need to specify current parametrization generation")
        quit()
...
#                continue
 ...
 #           else:
                ...
                if (fix_d == 1):
                   sublist = ("{}.xyz -N {} -sigma_scale 1.0 -o vdw -T 298 -t_A 1E5 -t 20E5 -T_A 100 -d_N {} -gens {} --molecule -FF".format(inchi,N_mol,d_N_fix,c['gens'])).split()
                ...
                # Else, run NPT
                   ...
                   sublist = ("{}.xyz  -N {} -sigma_scale 1.0 -o vdw -T 298 -t_A 1E5 -t 20E5 -T_A 100 -gens {} --molecule -FF".format(inchi,N_mol,c['gens'])).split()
                   sublist.append("{}".format(c['ff']))
                   gen_md_for_sampling.main(sublist)
 ...
 #           else:
        ...
        sublist = "vdw -r_min_scale 1.5 -every 10 -N 500 -p {} {} -QC_types dft -f {} --remove_cross -FF".format(c['vdw_qc_procs'],c['nod3'],c['functional']).split()
        # here needs to be previous gen drude so that it knows what pairs need to be generated
        #sublist.append(c['ff'])
        sublist.append(' '.join(['{}/gen{}_drude.db'.format(polar_dir,_) for _ in range(1,args.current_gen)]))
...
def orca_submit_bundle(c):
   jobids = []
   substring = "python3 /depot/bsavoie/data/Lin/taffi_beta/Automation_Scripts/orca_submit.py  -p 16 -t 1 -ppn 16 -q standby -sched slurm-negishi -size 1 -path_to_exe /depot/bsavoie/apps/orca_4_1_2/orca  --silent"

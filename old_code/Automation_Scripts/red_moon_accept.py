...
def main(argv):
    ...
    parser.add_argument('-w', dest='weight', default=1,
                        help = 'weight type 1~4')


    args=parser.parse_args()    
    sys.stdout = Logger(current_dir+'/red_moon.log')
    run_red_moon(restart=args.restart_flag,weight_type=int(args.weight))
    quit()
...
def run_red_moon(restart=False,weight_type=1):
    ...
    while cycle < tot_cycle:
      ...
      weight = [ nmol[0]*nmol[2]*rate_const[0]*np.exp(-beta*ER[0]), nmol[1]*nmol[1]*rate_const[1]*np.exp(-beta*ER[1]) ]
      print("weight: {:5.4f}:1".format(weight[0]/weight[1]))
      [rxn] = random.choices([0,1], weights=weight, k=1) 
      print("rxn: {} chosen!".format(rxn))
      pair = find_close(atom,rxn)
      ...
      os.rename("{}_switch.gro".format(cycle), "{}.gro".format(cycle+1))
      g = open('conc.txt','a') 
...
def write_nvtsh(cycle,switch_flag,min_flag=False):
   ...
   with open('nvt.sh','w') as f:
      ...
      if min_flag:
         ...
         f.write("$GRO mdrun -s em.tpr -deffnm ${title}\n")
         ...
         if atom[a]['resi'] == 'HYI' and atom[a]['elem'] == 'I':
            for b in atom:
               if atom[b]['resi'] == 'HYI' and atom[b]['elem'] == 'I' and atom[b]['mol_id'] != atom[a]['mol_id']:
                  ...
                  d_all.append(np.linalg.norm(d)) 
                  pair_all.append((a,b))
    return pair_all[d_all.index(min(d_all))]

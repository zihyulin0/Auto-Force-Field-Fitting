...
echo -e "#SBATCH --job-name ${outputname}" >> ${name_sub}.submit 
if [ ${ppn} -eq 0 ]; then
    echo -e "#SBATCH -N ${num_nodes}" >> ${name_sub}.submit 
else
    echo -e "#SBATCH -n $((${num_nodes}*${ppn}))" >> ${name_sub}.submit 
fi
if [ ${min_flag} -eq 1 ]; then
    echo -e "#SBATCH -t 00:${walltime}:00" >> ${name_sub}.submit 
else
    echo -e "#SBATCH -t ${walltime}:00:00" >> ${name_sub}.submit 
fi
echo -e "#SBATCH -A ${queue}" >> ${name_sub}.submit
echo -e "#SBATCH -o ${outputname}.out" >> ${name_sub}.submit 
echo -e "#SBATCH -e ${outputname}.err" >> ${name_sub}.submit 
...
echo -e "module load gcc/9.3.0 \n" >> ${name_sub}.submit
echo -e "module load openmpi/3.1.4 \n" >> ${name_sub}.submit
echo -e "module load ffmpeg/4.2.2 \n" >> ${name_sub}.submit
echo -e "module load  openblas/0.3.8 \n" >> ${name_sub}.submit
echo -e "module load  gsl/2.4 \n" >> ${name_sub}.submit
...
if [ ${duplicates} -eq 1 ]; then
	...
	sbatch ${name_sub}.submit -H    
    elif [ "${id}" != ""  ]; then       # Submit with job dependency	
	sbatch --depend=afterany:${id} ${name_sub}.submit
    else                                # Submit with no dependence
	sbatch ${name_sub}.submit
    fi

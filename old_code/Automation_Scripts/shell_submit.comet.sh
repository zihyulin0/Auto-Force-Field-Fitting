...
if [ ${ppn} -eq 0 ]; then
    ...
    echo -e "#SBATCH --nodes=${num_nodes}" >> ${name_sub}.submit 
    echo -e "#SBATCH --ntasks-per-node=${ppn}" >> ${name_sub}.submit 
fi
...
echo -e "#SBATCH -p ${queue}" >> ${name_sub}.submit
echo -e "#SBATCH -A ${account}" >> ${name_sub}.submit

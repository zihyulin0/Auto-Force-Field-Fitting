...
import sys,os,argparse,subprocess,shutil,time,glob,getpass,json,fnmatch,codecs
...
# This is the version of non parallel, it will just do inchikeys one by one
...
def main(argv):
    ...
    for g in gens:
      ...
      for inchi in keys:
         ...
         if os.path.isdir(current_dir+'/'+args.folder) is False:
            os.makedirs(current_dir+'/'+args.folder)
    ...
    """
    #keys = [ _ for _ in os.listdir(args.nonpolar_dir) if os.path.isdir('{}/{}'.format(args.nonpolar_dir,_)) and _ != 'already_inchi' ] 

    #batch = '/scratch/negishi/lin1209/Drude_0413_caleman/batch2'
    #inchi_here = [ _ for _ in os.listdir(batch) if os.path.isdir('{}/{}'.format(batch,_)) ]

    inchi_here = []
    with open('/depot/bsavoie/data/Lin/taffi_bumjoon_data/batch2_inchi','r') as f:
        for lc,lines in enumerate(f):
            fields = lines.split()
            inchi_here.append(fields[0])
    

    other = '/scratch/negishi/lin1209/Drude_0216_23_wB97X/polar_copy'
    inchi_other = [ _ for _ in os.listdir(other) if os.path.isdir('{}/{}'.format(other,_)) ] 
    already = '/scratch/negishi/lin1209/Drude_0413_caleman/polar_copy'
    inchi_already = [ _ for _ in os.listdir(already) if os.path.isdir('{}/{}/fitcharge_0/data_0.5'.format(already,_))]
    inchi_other_total = inchi_other + inchi_already
    
    keys = []
    for i in inchi_here:
        if i not in inchi_other_total:
            keys.append(i)
...
def write_status():
    ...
    status = {'place_charge':False, 'QM_polar': False, 'CHELPG_pot': False, 'Connolly_revaluation':False,'rewrite_pot':False,'write_charmm_data':False,'tar_folder':False}
    json.dump(status,codecs.open('status.json', 'w', encoding='utf-8'),indent=10)
...
def fit_Drude_custom(driver_dir,folder,args,tag):
    ...
    if os.path.isfile('status.json') is False:
      write_status()

    job_status = read_alljson('status.json')
    print("running: {}".format(folder))
    print("job status:")
    for i in job_status:
        print("{}: {}".format(i,job_status[i]))

    if job_status['write_charmm_data'] is True:
       print("Drude QM calculation for {} succeed!".format(folder))
    ...
    if job_status['place_charge'] is False:
         place_charge_block(nconf,c_config,job_status) 
    ...
    if job_status['QM_polar'] is False:
         orcaids = QM_polar_block(nconf,c_config,job_status)
    else: orcaids = []
    
    print("Starting CHELPG potential calculation...")
    if job_status['CHELPG_pot'] is False:
         chelpg_block(nconf,c_config,job_status)

    print("Starting reevaluation of potential at Connolly grid...")
    if job_status['Connolly_revaluation'] is False:
         reevaluation_block(nconf,c_config,job_status)
   
    print("Starting rewrite Connolly potential format...")
    if job_status['rewrite_pot'] is False:
         rewrite_connolly_block(nconf,c_config,job_status)
   
    print("Starting writing charmm data...") 
    if job_status['write_charmm_data'] is False:
         write_charmm_data_block(nconf,c_config,job_status)

    # tar configs files to save space
    """
    if job_status['tar_folder'] is False:
       for count_i,i in enumerate(list(range(0,nconf))):
         fitfolder = 'fitcharge_{}'.format(i)
         tar_file(fitfolder)
       job_status['tar_folder'] = True
       json.dump(job_status,codecs.open('status.json', 'w', encoding='utf-8'),indent=10)
    """
...
def place_charge_block(nconf,c_config,job_status):
    # try five times incase something went wrong
    for retry in range(0,3):
       # split into three batches in case too many jobs crashed the server
       batchsize = 100
       total_batch = ( (nconf-1)//batchsize ) + 1
       for batch in range(total_batch):
          jobids = []
          for count_i,i in enumerate(list(range(batchsize*batch,batchsize*(batch+1)))):
            if i == nconf: break
          ...
          if jobids == []: continue
          print("submitting placing charges: retry: {}, batch: {} out of {}".format(retry, batch, total_batch))
          monitor_jobs(jobids,user)
    clean_pcsubmit(nconf)

    if jobids == []:
       job_status['place_charge'] = True
       json.dump(job_status,codecs.open('status.json', 'w', encoding='utf-8'),indent=10)

    return

def QM_polar_block(nconf,c_config,job_status):
    for count_i,i in enumerate(list(range(0,nconf))):
      fitfolder = 'fitcharge_{}'.format(i)
      xyz = '{}/{}/{}.xyz'.format(c_config,i,i)
      element,geo = xyz_parse(xyz)
      in_name = polar_orca(fitfolder,geo,element)
      # check completion
      out_name = '{}.out'.format(in_name.split('.in')[0])
      if os.path.isfile(out_name) and check_completion(out_name) is False:
         os.remove(out_name)
  
    os.chdir(run_dir) 
    substring = "{}/Automation_Scripts/orca_submit.py -f polar.in -p {}  -t 1 -ppn {} -q {} -sched {} -size {} -o optimizations -path_to_exe {}  --silent"
    substring = substring.format(c["taffi_path"],c['charges_qc_procs'],c["charges_qc_ppn"],c["charges_qc_q"],c["charges_qc_sched"],c["charges_qc_size"],c["orca_exe"])
    orcaids = execute_orca_submit(substring)

    if orcaids == []:
       job_status['QM_polar'] = True
       json.dump(job_status,codecs.open('status.json', 'w', encoding='utf-8'),indent=10)
...
def chelpg_block(nconf,c_config,job_status):
       ...
       for q in [-0.5,0.5]:
          ...
          for batch in range(total_batch):
             ...
             while jobids != []:
                ...
                print("submitting chelpg potential for q={}: retry: {}, batch: {} out of {}".format(q,retry, batch, total_batch))
                monitor_jobs(jobids,user)
    ...
    if jobids == []:
       job_status['CHELPG_pot'] = True
       json.dump(job_status,codecs.open('status.json', 'w', encoding='utf-8'),indent=10)

    return

def reevaluation_block(nconf,c_config,job_status):
       ...
       for q in [0.5,-0.5]:
          ...
          for batch in range(total_batch):
             ...
             while jobids != []:
                ...
                print("submitting connollu revaluation for q={}: retry: {}, batch: {} out of {}".format(q,retry, batch, total_batch))
                monitor_jobs(jobids,user)
       clean_connolly(nconf)

    if jobids == []:
       job_status['Connolly_revaluation'] = True
       json.dump(job_status,codecs.open('status.json', 'w', encoding='utf-8'),indent=10)

    return


def rewrite_connolly_block(nconf,c_config,job_status):
    for q in [-0.5,0.5]:
       for count_i,i in enumerate(list(range(0,nconf))):
         fitfolder = 'fitcharge_{}'.format(i)
         if os.path.isfile('{}/rewrite.out'.format(fitfolder)):
            os.remove('{}/rewrite.out'.format(fitfolder)) 
         with open('{}/rewrite.in'.format(fitfolder),'w') as f:
            f.write("#!/bin/bash\n")
            f.write("python {}/Automation_Scripts/rewrite_pot_wrapper.py {} {} {} {} > rewrite.out \n".format(c['taffi_path'],fitfolder,q,c_config,run_dir))
            f.write("\n")
         # original
         #fitfolder = 'fitcharge_{}'.format(i)
         #xyz = '{}/{}/{}.xyz'.format(c_config,i,i)
       ...
       os.chdir(run_dir)
       # Submit orca job
       substring = "python3 {}/Automation_Scripts/bundle_submit.py  -p {} -t 30^min -ppn {} -q {} -sched {} -size {} -o charges -path_to_exe {}  --silent"
       substring = substring.format(c["taffi_path"],1,c["charges_qc_ppn"],c["charges_qc_q"],c["charges_qc_sched"],c["charges_qc_ppn"],c["orca_exe"])
    ...
    #   for count_i,i in enumerate(list(range(0,nconf))):
    #     fitfolder = 'fitcharge_{}'.format(i)
    #     rewrite_pot_clean(fitfolder,'_{}'.format(q))

    job_status['rewrite_pot'] = True
    json.dump(job_status,codecs.open('status.json', 'w', encoding='utf-8'),indent=10)

    return

def write_charmm_data_block(nconf,c_config,job_status):
    # Combine data to data_ dir
    for q in [-0.5,0.5]:
       for count_i,i in enumerate(list(range(0,nconf))):
       #for count_i,i in enumerate(list(range(15,nconf))):
         fitfolder = 'fitcharge_{}'.format(i)
         xyz = '{}/{}/{}.xyz'.format(c_config,i,i)
         element,geo = xyz_parse(xyz)
         total_pc,pc_layer1,pc_layer2,pc_layer3 = get_pc(fitfolder)
    ...
    job_status['write_charmm_data'] = True
    json.dump(job_status,codecs.open('status.json', 'w', encoding='utf-8'),indent=10)
...
def polar_orca(calc_dir,Geometry,Elements):
   ...
   with open("polar.in",'w') as f:
      ...
      for count_i,i in enumerate(Geometry):
         ...
         f.write("\n")
      f.write("*\n")
   return '{}/{}/polar.in'.format(run_dir,calc_dir)
...
def calc_CHELPG(calc_folder,xyz,total_pc,pc_layer1,pc_layer2,pc_layer3,geo,element,Qmag,tag):
    ...
    for i in range(0,total_pc):
         if os.path.isdir(str(i)) is False: os.makedirs(str(i))
            ...
            if  os.path.isfile(scfpfile) is True and check_completion('{}/charge.out'.format(i)): 
               continue
         os.chdir(str(i))
         ...
         Write_charge_in(name,geo,element,c["charges_qc_procs"])

         # clean incomplete output file
         ob_files = [ _ for _ in os.listdir() if len(_.split('_')) > 1] 
         if os.path.isfile('charge.out'):
            os.remove('charge.out')
         for j in ob_files:
            os.remove(j)
    ...
    if  os.path.isfile(scfpfile) is False: 
       ...
       Write_charge_in('unperturbed_charges',geo,element,c["charges_qc_procs"])
    ...
    # Submit orca job
    substring = "python3 {}/Automation_Scripts/orca_submit.py  -p {} -t 5^min -ppn {} -q {} -sched {} -size {} -o charges -path_to_exe {}  --silent"
    substring = substring.format(c["taffi_path"],c["charges_qc_procs"],c["charges_qc_ppn"],c["charges_qc_q"],c["charges_qc_sched"],c["charges_qc_size"],c["orca_exe"])
    jobids = execute_orca_submit(substring)
...
def check_completion(outputfile):
    complete_flag = False
    if os.path.isfile(outputfile) is False: return complete_flag
...
def Write_charge_in(name,geo,element,procs):
   procs = int(procs)
   with open('charge.in','w') as f:
      ...
      f.write('! {} {} TIGHTSCF {} CHELPG KeepDens\n\n'.format(c['functional'],c['basis'],D3_option))
      if procs > 1:
        f.write("\n%pal nprocs {}\n".format(procs))
        f.write("end\n")
      f.write('%base "{}"\n\n'.format(name))
...
# This deosn't work
def rewrite_pot_clean(calc_folder,tag):
    config_folder = 'configs_connolly{}'.format(tag)
    tmp_folder = '{}/{}/{}'.format(run_dir,calc_folder,config_folder)
    os.chdir(tmp_folder)
    folders = [ _ for _ in os.listdir(tmp_folder) if os.path.isdir('{}/{}'.format(tmp_folder,_)) ]
    for i in folders:
         if os.path.isfile('{}/{}_charges.vpot'.format(i,i)) is True: 
            ob_files = ['{}/rewrite.in'.format(i),'{}/rewrite.out'.format(i)]
            for j in ob_files:
               os.remove(j)
    return
...
def tar_file(folder):
    ...
    for i in folder_names:
      ...
      time.sleep(5)
      if os.path.isfile('{}.tar.gz'.format(i)):
         subprocess.call('rm -r  {}'.format(i),shell=True)
    subprocess.call('tar zcvf pointcharge.tar.gz pointcharge_layer*',shell=True)
    time.sleep(5)
    if os.path.isfile('pointcharge.tar.gz'):

    ...
    for g in gens: 
      ...
      for mc in mc_keys:
         ...
         if os.path.isfile('{}/{}/vdw/vdw_drude/extract_vdw.log'.format(nonpolar_dir,mc)) is False:
            ...
            continue
         with open('{}/{}/vdw/vdw_drude/extract_vdw.log'.format(nonpolar_dir,mc)) as f:
            for lc,lines in enumerate(f):
               fields = lines.split()
               if len(fields) < 4: continue
               if fields[0] == 'Initial' and fields[1] == 'xhi^2' and fields[2] == '(kcal/mol):': 
                  flag += 1
                  ini = float(fields[3])
               if fields[0] == 'Final' and fields[1] == 'xhi^2' and fields[2] == '(kcal/mol):': 
                  flag += 1
                  final = float(fields[3])
               if flag == 2: break
         with open('{}/{}/vdw/cycle-20/extract_vdw.log'.format(nonpolar_dir,mc)) as f:
            for lc,lines in enumerate(f):
               fields = lines.split()
               if len(fields) < 4: continue
               if fields[0] == 'Final' and fields[1] == 'xhi^2' and fields[2] == '(kcal/mol):': 

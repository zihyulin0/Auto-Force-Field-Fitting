...
def fit_Drude(c,tag):
    ...
    with open(FinalFF,'w') as f:
        f.write("#\n")
    #shutil.copyfile(intraonly,FinalFF) # now doing refit, don't copy intra 
    ...
    # 3. intra refit: extract_intra: VDWonly, chargeonly, lower gens' local Drude, Drude charge just fitted (genN_drude.db)
    #
    # FF info:
    # 1. in each inchikey:
    ...
    for g in gens: 
      #shutil.copyfile(intraonly,'{}/gen{}_drude.db'.format(driver_dir,g)) # now doing refit, don't copy intra
      with open('{}/gen{}_drude.db'.format(driver_dir,g),'w') as f:
        f.write("#\n")

      option = {'master':'{}/gen{}_drude.db'.format(driver_dir,g),'no_ct':False,'replace':True}
      ...
      for inchi in mc_keys:
         ...
         # drude_Q.db only contains charge and polar
      ...
      # now genN_drude.db has Drude charges
      # refit intra after charge
      fit_params(g,c,status,'after_charge')
      # the intra parameter after this will not be merged, but will be used as input for VDW_re
      ...
      for inchi in mc_keys:
         ...
         # DFT-AA.db only contains vdw
         option['new_params'] = '{}/{}/vdw/vdw_drude{}/DFT-AA.db'.format(nonpolar_dir,inchi,tag)
         merge_FF_drude.fun(option)
         
      # now genN_drude.db has this gen's Drude charges and Drude vdw (and last generation's Drude charge,vdw,intra)
      # refit intra after vdw
      fit_params(g,c,status,'final_fit')

      # the intra has been merged in genN_drude.db in fit_params

    ### Final Merge
    option = {'master':FinalFF,'no_ct':True,'replace':True}
    for g in gens: 
...
def fit_params(g,c,status,stage):


    # Find inchi keys for this generation
    mc_keys = [ _ for _ in status["mc"].keys() if int(g) in status["mc"][_]["gen"] ]
    # case: after_charge
    # Charge force field files are read. 

    if stage == "after_charge":
    # previous gen's genN_drude.db will have the Drude charge, Drude vdw, Drude refit intra
    # this gen's genN_drude.db will have the Drude charge
        ori_dir = os.getcwd()
        os.chdir(nonpolar_dir)

        sublist = ['{}/Automation_Scripts/shell_submit.sh'.format(c['taffi_path'])]
        sublist.append('python {}/FF_functions/extract_intramolecular_params_drude.py -f \'{}\' -o gen-{}_after_charge_drude'.format(c["taffi_path"]," ".join(mc_keys),g)+\
                       ' -FF \'{} {} {}\''.format(VDWonly,chargeonly," ".join(["{}/gen{}_drude.db".format(driver_dir,_) for _ in range(1,g+1)]))+\
                       ' -min_cycles 5 -max_cycles 10 -N_sc 3 -lammps_exe {} -gens {} --mixing_rule wh -batch {}'.format(c["lammps_exe"],c['gens'],g))
        sublist= sublist + ('after_charge  -p 1 -t {} -q {} -ppn 1'.format(c["param_fit_wt"],c["param_fit_q"])).split()
        ...
        if os.path.isdir("gen-{}_after_charge_drude".format(g)) is False or os.path.isfile("gen-{}_after_charge_drude/params-DFT.db".format(g)) is False:
            print("An error occured during the gen {} after charge fit.".format(g))
            quit()

        os.chdir(ori_dir)
        # Don't merge yet, but this parameter file will be used during VDW refit
    ...
    if stage == "final_fit":
    # previous gen's genN_drude.db will have the Drude charge, Drude vdw, Drude refit intra
    # this gen's genN_drude.db will have the Drude charge and Drude vdw
    # the command here is the same as after charge but at this point the current gen's genN_drude.db has been updated to have the new vdw parameters
        ori_dir = os.getcwd()
        os.chdir(nonpolar_dir)

        sublist = ['{}/Automation_Scripts/shell_submit.sh'.format(c['taffi_path'])]
        sublist.append('python {}/FF_functions/extract_intramolecular_params_drude.py -f \'{}\' -o gen-{}_final_params_drude'.format(c["taffi_path"]," ".join(mc_keys),g)+\
                       ' -FF \'{} {} {}\''.format(VDWonly,chargeonly," ".join(["{}/gen{}_drude.db".format(driver_dir,_) for _ in range(1,g+1)]))+\
                       ' -min_cycles 5 -max_cycles 10 -N_sc 3 -lammps_exe {} -gens {} --mixing_rule wh -batch {}'.format(c["lammps_exe"],c['gens'],g))
        sublist= sublist + ('final_params  -p 1 -t {} -q {} -ppn 1'.format(c["param_fit_wt"],c["param_fit_q"])).split()
        output = subprocess.Popen(sublist,stdout=subprocess.PIPE,stderr=subprocess.PIPE).communicate()[0]
        output = str(output,'utf-8')
        jobids = [ output.split("\n")[-2].split()[-1]]
        ...
        # Check completion and update status/all.json        
        if os.path.isdir("gen-{}_final_params_drude".format(g)) is False or os.path.isfile("gen-{}_final_params_drude/params-DFT.db".format(g)) is False:
        ...
        option = {'no_ct':False,'replace':True}
        option['master'] = '{}/gen{}_drude.db'.format(driver_dir,g)
        option['new_params'] = '{}/gen-{}_final_params_drude/params-DFT.db'.format(nonpolar_dir,g)
        merge_FF_drude.fun(option)
...
def write_fitinp(run_dir,fitfolder,npert,mag_c,current_gen,tag,c_xyz):
    ...
    if os.path.isfile(fitfolder+'/fitcharge{}.inp'.format(tag)) is False:
      ...
      # append lower gens' local Drude FF, lower gens' local Drude FF has: drude charge, drude refit vdw, drude refit intra
...
def VDW_re(run_dir,args,tag,inchikey,current_gen,xyz):
    ...
    if os.path.isdir(vdwfolder+'/'+FFname) is False:
      ...
      # append lower gens' local Drude FF and the this generation's refit intra and this inchi's just fit Drude charge
      FFstring = '{} {} {} {} {} {}/drude_Q.db'.format(intraonly,chargeonly,VDWonly,' '.join(['{}/gen{}_drude.db'.format(driver_dir,i) for i in range(1,current_gen)]),'{}/gen-{}_after_charge_drude/params-DFT.db'.format(nonpolar_dir,current_gen),run_dir)
      sublist.append(FFstring)
    ...
    # merge previous generation drude just in case, this shouldn't have any effect as this only serves as initial guess
    ...
    if os.path.isdir(vdwfolder+'/'+vdwdrude_dir) is False:
       ...
       this_gen_after_charge_intra = '{}/gen-{}_after_charge_drude/params-DFT.db'.format(nonpolar_dir,current_gen)
       charge_FF = '{}/drude_Q.db'.format(run_dir) 
       sublist = ['{}/Automation_Scripts/shell_submit.sh'.format(c['taffi_path'])]
       if tag == '':
         sublist.append('{}/FF_functions/extract_vdw_drude.py -f vdw -FF_DFT \'{} {} {}\' -o {} -E_max 0.0 -xhi2_thresh 1E-8 -q_a 0.0 -q_b 0.0 -mixing_rule wh -L2_sigma {} -L2_eps {} -fun {} '.format(c['taffi_path'],FF_path,this_gen_after_charge_intra,charge_FF,vdwdrude_dir,L2_s,L2_e,c['functional']))
       else:
         sublist.append('{}/FF_functions/extract_vdw_drude.py -f vdw -FF_DFT \'{} {} {}\' -o {} -E_max 0.0 -xhi2_thresh 1E-8 -q_a 0.0 -q_b 0.0 -mixing_rule wh -L2_sigma {} -L2_eps {} -fun {} -tag {}'.format(c['taffi_path'],FF_path,this_gen_after_charge_intra,charge_FF,vdwdrude_dir,L2_s,L2_e,c['functional'],tag))

...
def run_red_moon(restart=False):
    ...
    while cycle < tot_cycle:
      ...
      cand = find_cand(atom)
      # if the previous pair rxn is rejected, then we would have the same configuration
...
def write_nvtsh(cycle,switch_flag,min_flag=False):
   ...
   with open('nvt.sh','w') as f:
      ...
      f.write("title={}\n".format(title))
      if min_flag:
         f.write("$GRO grompp -f em_out.mdp -c ${title}.gro -o em.tpr\n")
         f.write("$GRO mdrun -s em.tpr -deffnm em\n")
         f.write("rm mdout.mdp\n")
         f.write("$GRO grompp -f nvt.mdp -c em.gro -o nvt.tpr -maxwarn 1\n") 
         f.write("$GRO mdrun -s nvt.tpr -deffnm ${title} -nt 8\n")
         f.write("rm em.*\n")
         f.write("rm ${title}.xtc\n")
      else:
         f.write("titlenext={}\n".format(cycle+1))
         f.write("$GRO grompp -f nvt.mdp -c ${title}.gro -o nvt.tpr -maxwarn 1\n") 
         f.write("$GRO mdrun -s nvt.tpr -deffnm ${titlenext} -nt 8\n")
    ...
    with open(filename,'r') as f:
       for lc,lines in enumerate(f):
         ...
         if lc == tot_atom+2:
            box = float(fields[0])
            continue
         atom_count = int(fields[2])-1
         atom[atom_count] = {}
         atom[atom_count]['mol_id'],atom[atom_count]['resi'] = resi_gro(fields[0])
         if is_number(fields[5]) is False:
            head = fields[5].split('.')[0]
            tail = split_string(fields[5].split('.')[1])[0:3] 
            fields[5] = '{}.{}'.format(head,''.join(tail))
         atom[atom_count]['coord'] = np.array([float(i)*10 for i in fields[3:6]])
         atom[atom_count]['elem'] = split_string(fields[1])[0] 
         atom[atom_count]['atom_name'] = fields[1]
         if atom[atom_count]['mol_id'] not in list(id_atom.keys()): 
            id_atom[atom[atom_count]['mol_id']] = [] 
         id_atom[atom[atom_count]['mol_id']].append(atom_count)
    return atom,id_atom
